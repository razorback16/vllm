...
class Qwen4ExpNGramEmbedding(PleOffloadLayer):
    ...
    def forward_impl(  # type: ignore[override]
        ...
        del hidden_states
        if is_offload_process():
            # The offload process is never CUDA-graph captured, so the
            # num_reqs-dependent ID generation can run inline.
        ...
        return self.embed_ngram_ids(ngram_ids, input_ids.shape[0], output_buffer)
    ...
    def embed_ngram_ids(
        ...
        """Look up PLE rows, writing into the offload buffer when given one."""
